...
train_data = DataLoader(dataset_train, num_workers=3, collate_fn=collater, batch_sampler=sampler)
print('train dataset len: {}'.format(len(train_data.dataset)))

# load validation data
dataset_val = CocoDataset(coco_root, set_name='val2017',
                          transform=transforms.Compose([Normalizer(), Resizer()]))
sampler_val = AspectRatioBasedSampler(dataset_val, batch_size=int(params.batch_size/2), drop_last=False)
